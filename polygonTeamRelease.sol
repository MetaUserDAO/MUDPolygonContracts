...
contract MudTeamReleaseBank {
    ...
    event icofinalized(uint256 icoDepositTotal);
    ...
    function releaseToken() external returns  (uint256, uint256) {
        ...
        require(bank[msg.sender].lastTime > 1734307200, "locking time incorrect!");//start to release after Date and time (GMT): Monday, 16 December 2024 00:00:00
    ...
    function icoFinalised() external returns (uint256) {
        ...
        _icoFinished = true;
        emit icofinalized(_icoDepositTotal);
        return _icoDepositTotal;
    }
}
